...
function ApiController() {
    this.path = '';
    this.model = null;
    //this.acl = { auth: false, role: null };
    this.qFields = [];
    this.fields = '';
    this.sFields = '';
    this.sort = null;//default sort option
    this.split = /\s|,|;/;
    this.partialOps = PartialOps;
    // create async methods for
    // var methods = ['begin','end','onBeforeSelect','onAfterSelect','onBeforePOST','onAfterPOST','onBeforePUT','onAfterPUT','onBeforePATCH','onAfterPATCH','onBeforeDELETE','onAfterDELETE','queryPipeline'];
    // methods.forEach(function(name){
    // this[name+'Async'] = Bluebird.promisify(this[name], this);
    // },this);
    
    // for req begin->end
    this.beginAsync = Bluebird.promisify(this.begin, this);
    this.endAsync = Bluebird.promisify(this.end, this);
    // for select docs begin->end
    this.onBeforeSelectAsync = Bluebird.promisify(this.onBeforeSelect, this);
    this.onAfterSelectAsync = Bluebird.promisify(this.onAfterSelect, this);
    // for select a doc begin->end
    this.onBeforeSelectOneAsync = Bluebird.promisify(this.onBeforeSelectOne, this);
    this.onAfterSelectOneAsync = Bluebird.promisify(this.onAfterSelectOne, this);
    // for count begin->end
    this.onBeforeCountAsync = Bluebird.promisify(this.onBeforeCount, this);
    this.onAfterCountAsync = Bluebird.promisify(this.onAfterCount, this);
    // for create doc begin->end
    this.onBeforeCreateAsync = Bluebird.promisify(this.onBeforeCreate, this);
    this.onAfterCreateAsync = Bluebird.promisify(this.onAfterCreate, this);
    // for update doc begin->end
    this.onBeforeUpdateAsync = Bluebird.promisify(this.onBeforeUpdate, this);
    this.onAfterUpdateAsync = Bluebird.promisify(this.onAfterUpdate, this);
    // for partial update doc begin->end
    this.onBeforePatchAsync = Bluebird.promisify(this.onBeforePartialUpdate, this);
    this.onAfterPatchAsync = Bluebird.promisify(this.onAfterPartialUpdate, this);
    // for delete doc begin->end
    this.onBeforeDeleteAsync = Bluebird.promisify(this.onBeforeDelete, this);
    this.onAfterDeleteAsync = Bluebird.promisify(this.onAfterDelete, this);
    
    // // //this.onFilterAsync = Bluebird.promisify(this.onFilter, this);
    this.onQueryPipeAsync = Bluebird.promisify(this.onQueryPipe, this);
}
